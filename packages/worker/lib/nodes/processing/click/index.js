...
const click = async (input, options, context) => {
    ...
    common_1.assert(rootAncestor, errors_2.NodeError.NEEDS_ROOT_ANCESTOR);
    common_1.assert(rootAncestor.parseJavascript, errors_2.NodeError.NEEDS_REAL_BROWSER);
    common_1.assert(rootAncestor.parsedLink || parentResult.html, errors_2.ParseError.LINK_MISSING);
    common_1.assert(node.selector, errors_2.ParseError.SELECTOR_MISSING);
    let renderedHtml;
    try {
        ...
        throw new errors_1.FlowError(errors_3.ClickError.COULD_NOT_CLICK);
    }
    common_1.assert(renderedHtml, errors_2.ParseError.ERROR_RENDERING_HTML);
    if (onLog)
