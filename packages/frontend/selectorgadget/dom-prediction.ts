...
class DomPredictionHelper {
    ...
    pathOf(elem) {
        let path = ''
        for (const e of Array.from(this.recursiveNodes(elem))) {
            if (e) {
    ...
    cssDiff(array) {
        ...
        for (const cssElem of Array.from(encoded_css_array)) {
    ...
    tokenizeCss(css_string) {
        ...
        for (const char of Array.from(this.cleanCss(css_string))) {
            if (skip) {
                skip = false
            } else if (char === '\\') {
                skip = true
    ...
    tokenizeCssForDiff(css_string) {
        ...
        for (const token of Array.from(this.tokenizeCss(css_string))) {
            block.push(token)
    ...
    decodeCss(string, existing_tokens) {
        ...
        let out = ''
        for (const character of Array.from(string.split(''))) {
    ...
    encodeCssForDiff(strings, existing_tokens) {
        ...
        for (const string of Array.from(strings)) {
            let out = new String()
            for (const token of Array.from(this.tokenizeCssForDiff(string))) {
    ...
    orderFromPriorities(priorities) {
        ...
        for (
            i = 0, end = priorities.length, asc = end >= 0;
            asc ? i < end : i > end;
            asc ? i++ : i--
        ) {
        ...
        for (
            i = 0, end1 = priorities.length, asc1 = end1 >= 0;
            asc1 ? i < end1 : i > end1;
            asc1 ? i++ : i--
        ) {
    ...
    simplifyCss(css, selected, rejected) {
        ...
        while (got_shorter) {
            got_shorter = false
            for (
                let i = 0, end = parts.length, asc = end >= 0;
                asc ? i < end : i > end;
                asc ? i++ : i--
            ) {
                ...
                this._removeElements(part, parts, first, selector => {
                    if (
                        this.selectorGets('all', selected, selector) &&
                        this.selectorGets('none', rejected, selector) &&
                        (selector.length < best_so_far.length ||
    ...
    _removeElements(part, parts, firstChar, callback) {
        ...
        for (
            j = look_back_index, end = part, asc = look_back_index <= end;
            asc ? j <= end : j >= end;
            asc ? j++ : j--
        ) {
            parts[j] = ''
    ...
    predictCss(s, r) {
        ...
        let union = ''
        for (const selected of Array.from(s)) {
    ...
    cssToXPathBlockHelper(css_block) {
        ...
        for (const current of Array.from(css_block)) {
        ...
        for (
            let i = 0, end = expressions.length, asc = end >= 0;
            asc ? i < end : i > end;
            asc ? i++ : i--
        ) {
            out += expressions[i]
...
export default DomPredictionHelper
